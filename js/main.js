...
function showStats(data) {
    ...
    if(data.status == 404) {
        err = true;
        errMessage = "<br />Project not exist.";
    }

    if(data.status == 403) {
        err = true;
        errMessage = "<br />Exceeded GitHub's rate limiting. Please try again in about an hour.";
    }

    if(data.length == 0) {
        err = true;
        errMessage = "<br />No releases available.";
    }
    ...
    if(err) {
        ...
        $.each(data, function(index, item) {
            ...
            html += "<hr class='release-hr'>" + "<h3><span class='glyphicon glyphicon-tag'></span>&nbsp;&nbsp;" +
        ...
        if(totalDownloadCount) {
            ...
            totalHTML += "<h2><span class='glyphicon glyphicon-download'></span>Total Downloads : " + formatNumber(totalDownloadCount) + "</h2>";
            totalHTML += "</div>";
